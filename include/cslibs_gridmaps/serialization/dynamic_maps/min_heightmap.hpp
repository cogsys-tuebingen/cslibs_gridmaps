...
struct convert<std::shared_ptr<cslibs_gridmaps::dynamic_maps::MinHeightmap<Tp, T, AllocatorT>>>
{
    static Node encode(const typename cslibs_gridmaps::dynamic_maps::MinHeightmap<Tp, T, AllocatorT>::Ptr &rhs)
    {
        Node n;
        if (!rhs)
            return n;
    ...
    static bool decode(const Node& n, typename cslibs_gridmaps::dynamic_maps::MinHeightmap<Tp, T, AllocatorT>::Ptr &rhs)
    {
        ...
        rhs.reset(new cslibs_gridmaps::dynamic_maps::MinHeightmap<Tp, T, AllocatorT>(
                    n[0].as<cslibs_math_2d::Pose2<Tp>>(), resolution, chunk_resolution, n[3].as<T>(), n[4].as<T>()));
