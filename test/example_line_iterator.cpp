...
int main(int argc, char *argv[])
{
...

    {
        ...
        for(std::size_t i = 0 ; i < ends.size() ; ++i) {
            ...
            cslibs_gridmaps::static_maps::algorithms::Bresenham<uchar> it({start.x, start.y},
            {end.x,end.y},
            {mask.cols, mask.rows},
                                                                               mask.ptr<uchar>() );

            cslibs_gridmaps::static_maps::algorithms::Bresenham<uchar> const_it({start.x, start.y},
            {end.x,end.y},
            {mask.cols, mask.rows},
                                                                                     mask.ptr<uchar>() );
            while(!it.done()) {
